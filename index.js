...
async function createTelegraphReport(title, contentNodes) {
    try {
        const accRes = await axios.get(`${TELEGRAPH_API}/createAccount`, {
            params: { short_name: 'IntelBot', author_name: 'Threat Intelligence Engine' }
        });
        ...
        console.error('Telegraph API Error:', err.message);
        return null;
    }
}
...
const ThreatService = {
    ...
    async getBreaches(domain = null) {
        ...
        const res = await axios.get(`${XON_BASE_URL}/breaches`, { params });
        return res.data;
    },
...
bot.start(async (ctx) => {
    const text = 
        ...
        `Powered by <b>Telegraf.js</b> & <b>XposedOrNot</b> API.\n\n` +
...
bot.command('analytics', async (ctx) => {
    ...
    try {
        ...
        // Build Telegraph document nodes
...
bot.command('status', (ctx) => {
    const isWebhook = Boolean(process.env.WEBHOOK_URL);
    ctx.replyWithHTML(
        `⚙️ <b>Bot Health Diagnostics</b>\n\n` +
        `• <b>Engine:</b> Telegraf.js\n` +
        `• <b>Connection:</b> <code>${isWebhook ? 'Webhook' : 'Long Polling'}</code>\n` +
...
// --- Universal Engine Initialization ---
...
if (WEBHOOK_URL) {
    ...
    }).then(() => console.log(`[BOT] Launched via Webhook on port ${PORT}`));
} else {
